#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File              : torch_train_multi_gpus.py
...
def main():
    ...
    model = model.cuda(device_ids[0])  # 模型放在主设备
    ...
    for epoch in range(start_epoch, hp.num_epochs + 1):
        ...
        for data in get_batch(hp.train_data_dir, batch_size=hp.batch_size, permission=hp.permission,
                              var_len=hp.var_len):
            x_batch = torch.Tensor(data[:, :-1, :]).cuda(device_ids[0])
            y_batch = torch.Tensor(data[:, 1:, :]).cuda(device_ids[0])
